...
**enable_oag_prerequisite_policies** | A boolean flag indicating whether or not policies are created for an Oracle Access Governance (OAG) instance and a group for the AGCS user in OAG to analyze OCI IAM permissions. Documentation on policies is here: [Set Up Identity Resources on OCI to Connect to Oracle Access Governance](https://docs.oracle.com/en/cloud/paas/access-governance/tjrtj/index.html#GUID-29D81CB5-08BB-45CB-8911-416F6FFDB0C9).| Yes | false
...
**existing_access_governance_group_name** | List of names and/or OCIDs of existing groups for the Oracle Access Governance's instance service account.  Spaces in the names are allowed. | No | None
