...
module "network_admins" {
  ...
  policy_statements     = ["Allow group ${module.network_admins.group_name} to manage virtual-network-family in compartment ${local.network_compartment_name}"]
}
# Security services
module "security_admins" {
  ...
  policy_statements     = ["Allow group ${module.security_admins.group_name} to manage vaults in compartment ${local.security_compartment_name}",
                           "Allow group ${module.security_admins.group_name} to manage keys in compartment ${local.security_compartment_name}",
                           "Allow group ${module.security_admins.group_name} to manage secret-family in compartment ${local.security_compartment_name}"]
}
...
module "compute_storage_admins" {
  ...
  group_description     = "Group responsible for managing compute instances and storage resources in compartment ${local.compute_storage_compartment_name}."
  user_ids              = []
  policy_compartment_id = var.tenancy_ocid
  policy_name           = "${var.service_label}-ComputeStorageAdmins-Policy"
  policy_description    = "Policy allowing ${var.service_label}-ComputeStorageAdmins group to manage instance-family and storage resources in compartment ${local.compute_storage_compartment_name}."
  policy_statements     = ["Allow group ${module.compute_storage_admins.group_name} to manage instance-family in compartment ${local.compute_storage_compartment_name}",
                           "Allow group ${module.compute_storage_admins.group_name} to manage volume-family in compartment ${local.compute_storage_compartment_name}",
                           "Allow group ${module.compute_storage_admins.group_name} to manage object-family in compartment ${local.compute_storage_compartment_name}",
                           "Allow group ${module.compute_storage_admins.group_name} to manage file-family in compartment ${local.compute_storage_compartment_name}"]
}
# Database service
...
module "appdev_admins" {
  ...
  policy_statements     = ["Allow group ${module.appdev_admins.group_name} to manage functions-family in compartment ${local.appdev_compartment_name}",
                           "Allow group ${module.appdev_admins.group_name} to manage api-gateway-family in compartment ${local.appdev_compartment_name}",
                           "Allow group ${module.appdev_admins.group_name} to manage ons-family in compartment ${local.appdev_compartment_name}",
                           "Allow group ${module.appdev_admins.group_name} to manage streams in compartment ${local.appdev_compartment_name}",
                           "Allow group ${module.appdev_admins.group_name} to manage cluster-family in compartment ${local.appdev_compartment_name}"]
}