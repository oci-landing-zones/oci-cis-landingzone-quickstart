...
locals {
  ...
  exainfra_admin_group_name      = var.deploy_exainfra_cmp ? (length(var.existing_exainfra_admin_group_name) == 0 && length(trimspace(var.rm_existing_exainfra_admin_group_name)) == 0  ? [module.lz_groups.groups[local.exainfra_admin_group_key].name] : (length(regexall("^ocid1.group.oc.*$", var.rm_existing_exainfra_admin_group_name)) > 0   ? ["'${data.oci_identity_group.existing_exainfra_admin_group.name}'"]  : [for grp in var.existing_exainfra_admin_group_name : "'${grp}'"])) : [for grp in var.existing_exainfra_admin_group_name : "'${grp}'"]
