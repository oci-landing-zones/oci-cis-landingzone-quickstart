...
locals {
  ...
  #----------------------------------------------------------------------------------
  iam_admin_group_name           = length(trimspace(var.existing_iam_admin_group_name)) == 0           ? module.lz_groups.groups[local.iam_admin_group_key].name           : (length(regexall("^ocid1.group.oc.*$", var.existing_iam_admin_group_name)) > 0           ? data.oci_identity_group.existing_iam_admin_group.name           : local.provided_iam_admin_group_name)
  cred_admin_group_name          = length(trimspace(var.existing_cred_admin_group_name)) == 0          ? module.lz_groups.groups[local.cred_admin_group_key].name          : (length(regexall("^ocid1.group.oc.*$", var.existing_cred_admin_group_name)) > 0          ? data.oci_identity_group.existing_cred_admin_group.name          : local.provided_cred_admin_group_name)
  security_admin_group_name      = length(trimspace(var.existing_security_admin_group_name)) == 0      ? module.lz_groups.groups[local.security_admin_group_key].name      : (length(regexall("^ocid1.group.oc.*$", var.existing_security_admin_group_name)) > 0      ? data.oci_identity_group.existing_security_admin_group.name      : local.provided_security_admin_group_name)
  ...
  database_admin_group_name      = length(trimspace(var.existing_database_admin_group_name)) == 0      ? module.lz_groups.groups[local.database_admin_group_key].name      : (length(regexall("^ocid1.group.oc.*$", var.existing_database_admin_group_name)) > 0      ? data.oci_identity_group.existing_database_admin_group.name      : local.provided_database_admin_group_name)
  appdev_admin_group_name        = length(trimspace(var.existing_appdev_admin_group_name)) == 0        ? module.lz_groups.groups[local.appdev_admin_group_key].name        : (length(regexall("^ocid1.group.oc.*$", var.existing_appdev_admin_group_name)) > 0        ? data.oci_identity_group.existing_appdev_admin_group.name        : local.provided_appdev_admin_group_name)
  auditor_group_name             = length(trimspace(var.existing_auditor_group_name)) == 0             ? module.lz_groups.groups[local.auditor_group_key].name             : (length(regexall("^ocid1.group.oc.*$", var.existing_auditor_group_name)) > 0             ? data.oci_identity_group.existing_auditor_group.name             : local.provided_auditor_group_name)
  announcement_reader_group_name = length(trimspace(var.existing_announcement_reader_group_name)) == 0 ? module.lz_groups.groups[local.announcement_reader_group_key].name : (length(regexall("^ocid1.group.oc.*$", var.existing_announcement_reader_group_name)) > 0 ? data.oci_identity_group.existing_announcement_reader_group.name : local.provided_announcement_reader_group_name)
  cost_admin_group_name          = length(trimspace(var.existing_cost_admin_group_name)) == 0          ? module.lz_groups.groups[local.cost_admin_group_key].name          : (length(regexall("^ocid1.group.oc.*$", var.existing_cost_admin_group_name)) > 0          ? data.oci_identity_group.existing_cost_admin_group.name          : local.provided_cost_admin_group_name)
  storage_admin_group_name       = length(trimspace(var.existing_storage_admin_group_name)) == 0       ? module.lz_groups.groups[local.storage_admin_group_key].name       : (length(regexall("^ocid1.group.oc.*$", var.existing_storage_admin_group_name)) > 0       ? data.oci_identity_group.existing_storage_admin_group.name       : local.provided_storage_admin_group_name)
  exainfra_admin_group_name      = var.deploy_exainfra_cmp ? ((trimspace(var.existing_exainfra_admin_group_name)) == 0 ? module.lz_groups.groups[local.exainfra_admin_group_key].name : (length(regexall("^ocid1.group.oc.*$", var.existing_exainfra_admin_group_name)) > 0 ? data.oci_identity_group.existing_exainfra_admin_group.name : local.provided_exainfra_admin_group_name)) : local.provided_exainfra_admin_group_name
}