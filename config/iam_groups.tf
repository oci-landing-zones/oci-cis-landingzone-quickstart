...
module "lz_groups" {
  ...
  groups = var.use_existing_iam_groups == false ? {
    (local.network_admin_group_name) = {
      description  = "CIS Landing Zone group for managing networking in compartment ${local.network_compartment_name}."
      user_ids     = []
      defined_tags = null
    },
    (local.security_admin_group_name) = {
      description  = "CIS Landing Zone group for managing security services in compartment ${local.security_compartment_name}."
      user_ids     = []
      defined_tags = null
    },
    (local.appdev_admin_group_name) = {
      description  = "CIS Landing Zone group for managing app development related services in compartment ${local.appdev_compartment_name}."
      user_ids     = []
      defined_tags = null
    },
    (local.database_admin_group_name) = {
      description  = "CIS Landing Zone group for managing databases in compartment ${local.database_compartment_name}."
      user_ids     = []
      defined_tags = null
    },
