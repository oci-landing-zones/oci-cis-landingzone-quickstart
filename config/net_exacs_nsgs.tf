...
locals {
  ...
  } if var.add_public_subnet_to_exacs_vcns == true }

  exacs_lbr_nsgs = { for k, v in module.lz_exacs_vcns.vcns : "${k}-lbr-nsg" => {
    vcn_id : v.id,
    ingress_rules : merge({
      ssh-ingress-rule : {
        is_create : length(var.dmz_vcn_cidr) == 0 && var.add_public_subnet_to_exacs_vcns == true,
    ...
    egress_rules : {
      clt-egress_rule : {
        is_create : true,
        description : "HTTP egress rule to ${k}-clt-nsg.",
        stateless : false,
        protocol : "6",
        dst      = "${k}-clt-nsg",
        dst_type = "NSG_NAME",
  ...
  exacs_clt_nsgs = { for k, v in module.lz_exacs_vcns.vcns : "${k}-clt-nsg" => {
    ...
    ingress_rules : merge({
      ...
      db-bastion-ingress-rule : {
        is_create : var.add_public_subnet_to_exacs_vcns == true,
        description : "DB ingress rule for ${k}-bastion-nsg.",
        stateless : false,
        protocol : "6",
        src : "${k}-bastion-nsg",
        src_type : "NSG_NAME",
      ...
      ons-ingress-rule : {
        is_create : length(var.dmz_vcn_cidr) == 0,
        description : "Enables the Oracle Notification Services (ONS) to communicate about Fast Application Notification (FAN) events.",
        stateless : false,
        protocol : "6",
	    ...
	    db-client-ingress-rule : {
        is_create : length(var.dmz_vcn_cidr) == 0,
        description : "Enable client connections to the database and Oracle Data Guard",
        stateless : false,
        protocol : "6",
        src : module.lz_exacs_vcns.subnets[replace(k,"-vcn","-${local.client_subnet_prefix}-snt")].cidr_block,
        src_type : "CIDR_BLOCK",
	    ...
	    ssh-client-ingress-rule : {
        is_create : length(var.dmz_vcn_cidr) == 0,
        description : "SSH ingress rule for client subnet.",
        stateless : false,
        protocol : "6",
        src : module.lz_exacs_vcns.subnets[replace(k,"-vcn","-${local.client_subnet_prefix}-snt")].cidr_block,
        src_type : "CIDR_BLOCK",
      ...
      { for cidr in var.onprem_cidrs : "db-onprem-ingress-rule-${index(var.onprem_cidrs, cidr)}" => {
        is_create : var.is_vcn_onprem_connected && length(var.dmz_vcn_cidr) == 0 && length(var.onprem_cidrs) > 0,
        description : "DB ingress rule for on-premises CIDR ${cidr}.",
        stateless : false,
        protocol : "6",
        src : cidr,
        src_type : "CIDR_BLOCK",
