...
module "lz_sch_audit_objStore_policy" {
  ...
  policies = {
    (local.sch_audit_policy_name) = {
      ...
      statements = [
                    <<EOF
                        Allow any-user to manage objects in compartment id ${module.lz_compartments.compartments[local.security_compartment.name].id} where all {
                        request.principal.type='serviceconnector',
                        target.bucket.name= '${module.lz_sch_audit_bucket[0].oci_objectstorage_buckets[local.sch_audit_bucket_name].name}',
                        request.principal.compartment.id='${module.lz_compartments.compartments[local.security_compartment.name].id}' }
                    EOF
                ]
    }
  }
