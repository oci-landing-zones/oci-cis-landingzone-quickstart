...
variable "dmz_number_of_subnets" {
  ...
  type    = number
  validation {
    condition     = var.dmz_number_of_subnets > 1 && var.dmz_number_of_subnets < 6
    error_message = "Number of subnets must be a number from 1 to 5."
  }
  description = "Number of subnets to be created in the DMZ VCN."
}
variable "dmz_subnet_size" {
  default = 4
  type    = number
  validation {
    condition     = var.dmz_subnet_size > 3 && var.dmz_subnet_size < 16
    error_message = "Invalid bit range for subnet."
  }
...
#   type = number
#   validation {
#       condition = var.number_of_spokes > 0
#       error_message = "Number of Spokes must be greater than 0."
#   }
# }

variable "spoke_vcn_cidrs" {
    type    = list(string)
    default = ["10.0.1.0/20","10.0.16.0/20","10.0.32.0/20"]
