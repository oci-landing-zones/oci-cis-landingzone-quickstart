...
locals {
  ...
  security_zones = { 
    for k, v in var.security_zones : k => {
      ...
      security_policies = coalesce(v.cis_level,"2") == "2" ? setunion(local.cis_1_2_L2,local.cis_1_2_L1,coalesce(v.security_policies,[])) : setunion(local.cis_1_2_L1,coalesce(v.security_policies,[]))
