...
locals {
  ...
  provisioning_group_names_t = var.use_existing_provisioning_group == false ? {for k in keys(local.enclosing_compartments) : k => {group_name:"${k}-prov-group"}} : {(local.unique_prefix) : {group_name: var.existing_provisioning_group_name}}
  ...
  cloud_guard_policy_name = "${local.unique_prefix}-cloud-guard-policy"
  os_mgmt_policy_name     = "${local.unique_prefix}-os-management-policy"
  vss_policy_name         = "${local.unique_prefix}-vss-policy"
}