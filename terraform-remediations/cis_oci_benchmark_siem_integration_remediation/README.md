# Foundational OCI CIS SIEM Integration Stack
...
Configuration on the SIEM side is a seperate task not covered by this Terraform code.
...
Variable Name | Description | Required | Default Value
--------------|-------------|----------|--------------
**homeregion** | Indicates if the deployment is in the home region. | Yes | None
...
The following permissions are needed to run the stack using the resource manager. Note: The example policies provided below are broad in nature to allow the stack to run under different situations. It is recommended to review the policies and make adjustments based on your security requirements.
...
`Allow group <Remediation Group> to manage stream-family in tenancy`  
`Allow group <Remediation Group> to manage dynamic-groups in tenancy`  
`Allow group <Remediation Group> to manage policies in tenancy`  
`Allow group <Remediation Group> to manage serviceconnectors in tenancy`  
`Allow group <Remediation Group> to manage groups in tenancy`  
`Allow group <Remediation Group> to read audit-events in tenancy`  
...
1. Under **Working directory select the directory ending with *cis_oci_benchmark_siem_integration_remediation*
![Working_Directory](images/select_working_dir.png)
1. Click Next
1. Enter the required variables  
![variables](images/stack_vars.png)  
1. Click Next
1. Click Next
1. Click Plan & Apply
...
When deploying in your home region, you should expect Terraform to create 6 resources. This can be validated by reviewing the Terraform Log after a successful Apply Job
...
The TF code requires terraform 1.3 or later. When using 3rd party tooling you need to ensure the matching terraform binaries are used.
