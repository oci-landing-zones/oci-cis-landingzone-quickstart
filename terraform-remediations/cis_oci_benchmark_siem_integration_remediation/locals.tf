...
locals {
  ...
  siem_info = {
    "Splunk" = {
      ...
      iam_policy_api_key            = "allow group ${local.groups_configuration.groups.STREAM-READ-GROUP.name} to use stream-pull in ${local.resource_location}"
      iam_policy_instance_principal = "allow dynamic-group ${local.dynamic_groups_configuration.dynamic_groups.STREAM-READ-DYN-GROUP.name} to use stream-pull in ${local.resource_location}"
    }
    "Stellar Cyber" = {
