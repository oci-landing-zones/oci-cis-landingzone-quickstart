...
locals {
  ...
  ocir_vault_secret_id_condition                = "where target.secret.id = '${local.ocir_auth_token_secret_ocid}'"
  ...
  ocir_login_command                            = <<-EOT
    ...
    command -v oci >/dev/null 2>&1 || { echo "OCI CLI is required to retrieve the OCI Registry auth token Secret." >&2; exit 1; }
    command -v python3 >/dev/null 2>&1 || { echo "python3 is required to decode OCI Secret bundle content." >&2; exit 1; }

    encoded_content="$(oci secrets secret-bundle get --secret-id "$OCIR_AUTH_TOKEN_SECRET_OCID" --query 'data."secret-bundle-content".content' --raw-output)" || {
      echo "Unable to retrieve the OCI Registry auth token from OCI Secret. Verify that the value is a Secret resource OCID beginning with ocid1.vaultsecret and that the deployment principal can read its secret bundle." >&2
      exit 1
    }

    [ -n "$encoded_content" ] || {
      echo "OCI Secret returned empty content for the OCI Registry auth token." >&2
      exit 1
    }

    ocir_token="$(printf '%s' "$encoded_content" | python3 -c 'import base64,sys; sys.stdout.write(base64.b64decode(sys.stdin.read().strip(), validate=True).decode("utf-8"))')"
